...
export default function Header() {
  ...
  return (
    <>
<header className="w-full md:px-10 px-2.5 cursor-pointer flex justify-between sticky top-0 opacity-0 py-10 max-md:py-6 animate-slideIn-text backdrop-blur border-black/10 dark:border-white/10 z-50">
        <Link href={"/"}>
          <p className="text-4xl stroked-text opacity-0 ps-2 animate-slideIn-text">
            <svg
              ...
              viewBox="0 0 69 79"
              fill="none"
              className="w-[25px] h-[40px] sm:w-[50px] sm:h-[50px]"
            >
              <path
      ...
      >
        <DrawerContent>
          {(onClose) => (
            <>
              <DrawerHeader><Link href={"/"}                     onClick={onClose} // 👈 this closes the drawer when clicked
>
          <p className="text-4xl ">
            ...
            >
              ...
              <DrawerFooter className="text-center text-sm opacity-20">
                © {new Date().getFullYear()} Siraj
